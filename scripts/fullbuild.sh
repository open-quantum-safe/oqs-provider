...
if [ ! -f "_build/oqsprov/oqsprovider.so" ]; then
   ...
   # possibly remove instruction to build for debugging: TBC 
   cmake -DCMAKE_BUILD_TYPE=Debug -DOPENSSL_ROOT_DIR=$(pwd)/.local -DCMAKE_PREFIX_PATH=$(pwd)/.local -S . -B _build && cmake --build _build
   if [ $? -ne 0 ]; then
